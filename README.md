...
設定画面の「起動時にGitHubで更新を確認する」で無効化できます。「今すぐ更新を確認」は自動確認がオフでも利用できます。

新版が見つかると設定画面に更新先のバージョンとリリースページへのリンクを表示します。現在のバージョンは、更新の有無や確認中・通信失敗にかかわらず表示します。鑑賞中にポップアップを出したり、ブラウザーを自動で開いたりすることはありません。
アプリを起動したままの定期確認は行いません。公開直後に確認する場合は「今すぐ更新を確認」を押してください。

1. 更新通知のリンクを押し、GitHub ReleasesのAssetsから `DisplayVeil-<バージョン>-win.zip` を取得します。
