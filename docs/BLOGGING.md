...
原稿には代替として、このリポジトリの `main` にある画像を参照する GitHub Raw URL を設定しています。そのまま使う場合は、先に画像を含む変更を公開し、各 URL が読者から開けることを確認してください。原稿作成時点では、リモートの画像・Release の公開状態は確認できていません。
...
この作業はローカル原稿と画像素材の作成までです。GitHub への push、Release の作成、Qiita への投稿・画像アップロードは行っていません。

## 他のブログへの転用

PNG と本文を利用できます。Qiita 固有の `:::note info` は通常の段落や引用へ置き換え、画像 URL は掲載先に合わせてください。
無料で利用できるソフトとして紹介していますが、ソースコードや画像の利用許諾条件は新たに定義していません。
