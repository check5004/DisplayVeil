...
- **展開してすぐ起動** — インストール・管理者権限は不要。テレメトリ・常駐サービスはありません。
- **更新を設定画面でお知らせ** — 起動時にGitHubで新版を確認します。自動確認はオフにできます。
...
| 操作バーが邪魔なとき | バー左側の状態表示、またはボタン以外の余白を左ドラッグして画面内で移動 |
| キーボード作業などを続ける | 操作中の上部バーで「固定する」。マウスが離れても開いたまま |
| 手動で黒く戻す | 操作中の上部バーで「暗く戻す」 |
| 黒幕を全部解除して設定に戻る | 黒い部分を右クリック →「すべて解除」、または操作バーの「全解除」 |
| 設定を開かず黒幕を全部解除 | トレイメニューの「すべて解除」、または全解除ショートカット |
...
操作バーの移動位置は、同じ鑑賞中は黒く戻して再び開いても維持されます。鑑賞を終了すると初期位置に戻ります。
...
起動時に、前回の確認から24時間以上経っていればGitHubの公開リリースを確認します。初期設定では有効です。
設定画面の「更新とリリースノート」から更新用モーダルを開けます。モーダル内の「起動時にGitHubで更新を確認する」で自動確認を無効化できます。「今すぐ更新を確認」は自動確認がオフでも利用できます。

新版が見つかると設定画面に更新先のバージョンを表示します。現在のバージョンは、更新の有無や確認中・通信失敗にかかわらず表示します。モーダルでは現在版と公開版を並べ、公開版のリリースノートをスクロールして読めます。表示する変更内容はそのリリース1件分で、途中のバージョンを含む累積差分ではありません。
変更内容が未取得の場合は、モーダルを開いたときにGitHubへ確認します。通信失敗時も、この起動中に取得した変更内容と既知の公開ページを利用できます。鑑賞中にモーダルを自動表示したり、ブラウザーを自動で開いたりすることはありません。
アプリを起動したままの定期確認は行いません。公開直後に確認する場合は「今すぐ更新を確認」を押してください。

1. 「更新とリリースノート」を開き、「GitHubでダウンロード」を押します。ブラウザーで開いたページの「Assets」から、モーダルに表示された `DisplayVeil-<バージョン>-win.zip` を取得します。「Source code」は実行用ではありません。
2. 起動中のDisplay Veilを終了し、新しいZIPを別フォルダーに「すべて展開」します。
3. 新しいフォルダーの `DisplayVeil.exe` を起動します。通常の設定は引き継がれます。ショートカットを作っていた場合はリンク先も変更してください。

ダウンロード・ファイル交換・再起動は手動です。更新機能がないv1.0.0からは、最初の1回はこのページのダウンロードリンクから新版を取得してください。
オフラインやGitHubの利用制限で確認できなくても、黒幕機能はそのまま使えます。通信先は更新確認用のGitHub APIです。利用状況・設定・画面内容を送信するテレメトリはありません。
...
- 設定と前回の更新確認時刻・検出したリリースのタグは `%LOCALAPPDATA%\DisplayVeil\settings.json` に保存します。更新確認以外の通信・テレメトリ・常駐サービスはありません。
